...
void BackprojectionLayer< Dtype >::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  const vector<Blob<Dtype>*>& top) {

 // just do an equal spacing of angles for now
 const Dtype start_angle  = this->layer_param_.backprojection_param().start_angle();
 const Dtype end_angle    = this->layer_param_.backprojection_param().end_angle();

 const Dtype step_width = ( end_angle - start_angle ) / bottom[0]->shape()[2];
 for( int i = 0; i < bottom[0]->shape()[2]; ++i ) {
  Dtype angle = (-(start_angle + i * step_width)*M_PI)/180.;
  this->angles.push_back( angle );
 }
}

template< typename Dtype >
void BackprojectionLayer< Dtype >::Reshape(const vector<Blob<Dtype>*>& bottom,
  const vector<Blob<Dtype>*>& top) {
...
template< typename Dtype >
void BackprojectionLayer< Dtype >::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  const vector<Blob<Dtype>*>& top) {
...
template< typename Dtype >
inline Dtype BackprojectionLayer< Dtype >::backprojectPoint( GridPositionType position, Dtype angle, PositionType detectorPosition,int detectorLength ) {
...
template< typename Dtype >
inline Dtype BackprojectionLayer< Dtype >::backprojectPoint( GridPositionType position, const std::vector< Dtype >& rotationMatrix , PositionType detectorPosition,int detectorLength ) {
...
template< typename Dtype >
void BackprojectionLayer< Dtype >::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(BackprojectionLayer);
#endif

INSTANTIATE_CLASS(BackprojectionLayer);
