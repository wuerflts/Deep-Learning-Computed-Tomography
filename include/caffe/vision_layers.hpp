...
template <typename Dtype>
class BackprojectionLayer : public Layer<Dtype> {
 public:
  explicit BackprojectionLayer(const LayerParameter& param)
  : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "Backprojection"; }
 ...
 private:
  ...
  typedef pair< int, int > GridPositionType;
  typedef pair< double, double > PositionType;

  std::vector< Dtype > angles;

  Dtype detector_spacing;

  inline Dtype backprojectPoint( GridPositionType position, Dtype angle, PositionType detectorPosition,int detectorLength );
  inline Dtype backprojectPoint( GridPositionType position, const std::vector< Dtype >& rotationMatrix, PositionType detectorPosition, int detectorLength );
  inline void  fillRotationMatrix( std::vector< Dtype >& rotationMatrix, Dtype angle )
  {
   rotationMatrix.push_back(cos(angle));
   rotationMatrix.push_back(sin(angle));
   rotationMatrix.push_back(-sin(angle));
   rotationMatrix.push_back(cos(angle));
  }
...
template <typename Dtype>
class FanBackprojectionLayer : public Layer<Dtype> {
 public:
  explicit FanBackprojectionLayer(const LayerParameter& param)
  : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
